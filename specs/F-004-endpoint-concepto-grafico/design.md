...
- `infrastructure/security/sql_write_guard.py`, `sql_query_guard.py`,
  `database_reference_guard.py`, `identifier_guard.py`: se usan, no se editan.
- `infrastructure/repositories/sql_server_repository.py`: bastan
  `execute_read_query`, `peek_next_ide` y `run_in_write_transaction`. No se
  añade método (la propuesta §15 preveía uno; no hace falta).
- `application/use_cases/execute_sql_command_use_case.py`,
  `read_document_use_case.py`, los modelos y casos de uso de albaranes,
  `write-lists.json`, `infra/`. El `GRANT` de la propuesta §12.2 no existe:
  los permisos están concedidos [MEDIDO].
...
class AttachConceptoGraficoRequest(BaseModel):
    ...
    usu: str = Field(..., min_length=1, max_length=24)   # usu.cod es Texto 24
    contenido_base64: str = Field(..., min_length=4)      # tope: ver R8
    sha256: str | None = Field(default=None, pattern=r"^[0-9a-fA-F]{64}$")
    commit: bool = False
```

`content_type` y `clave_idempotencia` de la propuesta §6.2 **no** se aceptan: el
tipo lo decide la firma binaria (se devuelve `content_type` detectado, como en
`documents/read`), y la clave no tiene dónde guardarse sin repurponer una
columna del ERP (pregunta Q5 en `requirements.md`). El tope de longitud del
base64 (`ceil(max_bytes*4/3)+4`) se comprueba en el guard, no en el modelo,
para que el modelo no dependa de `Settings`.
...
| E6 | commit | `INSERT INTO dbo.rcg (ide, con, gra, pos, cla) VALUES (?, ?, ?, ?, ?)` |
| E7 | commit | relectura: `SELECT COUNT(*) FROM <doc>.dbo.gra WHERE emp = ? AND cod = ?`, ídem `dbo.gra`, y `SELECT COUNT(*) FROM dbo.rcg WHERE ide = ?` |

Las 29 columnas de `gra` van **explícitas**, en una tabla única del módulo:

| Columna | Documental (E4) | Negocio (E5) |
|---|---|---|
| `ide` | reservado E1 | reservado E2 |
| `cod`, `emp` | generado una vez / `con.emp` | **los mismos** |
| `res` | valor medido en T1 (vacío en las filas recientes) | `request.res` |
| `nom`, `nomori` | `request.nom` | `request.nom` |
| `gratipide` | valor medido en T1 | `request.gratipide` |
| `vin`, `usu`, `fec` | `3`, `request.usu`, hoy `AAAAMMDD` | ídem |
| `ima` | el binario | `NULL` |
| `tex`, `cla`, `guid` | `''` | `''` |
| `estcon`, `tipocu`, `texrev`, `numrev`, `salfec`, `salhor`, `salusu`, `saltex`, `mntide`, `graant`, `anx`, `ori`, `pul`, `tip`, `cam` | constantes medidas en T1 sobre la pareja real (esperado `0`/`''`/`NULL`) | ídem |

Por qué explícitas y no clonando una fila plantilla (el patrón de albaranes):
`gra.graant` y `mntide` son referencias; clonar una fila cualquiera copiaría
la «versión anterior» de otro documento. Con columnas explícitas el SQL es
constante y el test lo compara carácter a carácter (R22).
...
`fec` sale del mismo instante. Q4 (qué significan los 4 dígitos) sigue abierta
y no bloquea: el formato se conserva.
...
run(request)
  ...
  ├─ lecturas L1-L5 con credenciales de LECTURA (R7, R17)
...
- **Reintento**: `run_in_write_transaction` captura `pyodbc.IntegrityError`,
  revierte y repite `work`. El `cod` es el mismo en cada intento (se generó
  fuera), y como el intento anterior se revirtió no colisiona con `(emp, cod)`.
  Una colisión real de `cod` —dos peticiones con el mismo segundo, usuario y
  `dddd`— se agota en los reintentos y sale como `colision_de_clave` (R15, R16).
- **`SET NOCOUNT ON`** lo deja activo el repositorio en la conexión al tomar
  el applock; por eso R14 se verifica **releyendo** (E7) y no con
  `cursor.rowcount`, que el driver puede devolver a `-1`.
- **Timeout**: el `timeout_seconds` de `_connect()` es de login, no de
  sentencia; `work` fija `cursor.connection.timeout` para que la aplicación
  falle (y revierta) antes de los 230 s del balanceador (propuesta §10).
...
- **`auxgra.tipaso`** se lee (L2) y se devuelve en `avisos` si no contiene
  ningún código, pero **no** se valida contra `con.tip`: no está medida la
  correspondencia (pregunta abierta). La lista blanca de `gratipide` cubre.
- **Auditoría** (R21): `logger.info` con un dict serializado; el `sha256`
  demuestra qué fichero se subió sin guardar el fichero.

## Tests sin red (R22)

- Repositorio falso con `execute_read_query` (devuelve por sentencia),
  `peek_next_ide` y `run_in_write_transaction` (invoca `work` con un cursor
  falso que graba `execute(sql, *params)` y responde a `fetchone`).
- El test de R15 hace que el cursor falso lance en E5 y comprueba que no se
  llegó a E6 y que el caso de uso no captura la excepción (el rollback es del
  repositorio real, ya probado en producción por los albaranes).
- Control negativo de R20: ninguna constante contiene `UPDATE|DELETE|MERGE|
  DROP|ALTER|CREATE|TRUNCATE` (regex sobre todas las sentencias del módulo).
- R19: `DatabaseReferenceGuard.extract_database_references(sql)` sobre cada
  sentencia devuelve `[]` o `["<documental>"]`, y `validate(...,
  allowed=["ruesma"])` **rechaza** E1/E4/E7/L5: prueba que el guardia sigue
  cerrando `sql/write` a lo que este endpoint sí puede hacer.

## Riesgos y decisiones

| Decisión | Alternativa descartada | Por qué |
|---|---|---|
| Idempotencia por contenido | Clave en `gra.guid` (propuesta §9.1) | `guid` es del ERP y Q5 está abierta; el contenido no repurpone nada |
| `cod` con formato de Sigrid | `cod` determinista = clave del cliente | Q4 abierta: si Sigrid interpreta el sello, un `cod` ajeno rompe la ficha |
...
| Sin fila en `dbo.log` | Escribirla como F-009 | No consta que Sigrid la escriba al importar (Q7); se invierte si la medición lo dice |
...
**Riesgo principal**: valores de columnas no variables mal fijados (`estcon`,
`ori`, `gratipide`/`res` documental). Mitigación: T1 mide una pareja real
antes de escribir el constructor, el dry-run enseña las filas completas, y el
primer commit se hace sobre una reclamación de prueba elegida por Posventa,
con `res = 'PRUEBA API - BORRAR'` y limpieza desde la UI de Sigrid (propuesta
§14, nivel 3). No hay `DELETE` ni debe haberlo.
...
## Preguntas abiertas (decide el humano; todas de solo lectura)

| # | Pregunta | Defecto del diseño | Consulta que la responde |
|---|---|---|---|
| Q4 | ¿Interpreta Sigrid el formato o los 4 dígitos de `gra.cod`? | Se conserva el formato medido; `dddd` sale del `sha256` | `SELECT TOP (200) cod FROM dbo.gra ORDER BY ide DESC` |
| Q5 | ¿Usa Sigrid `gra.guid`? ¿Vale la idempotencia por contenido (R17) sin `clave_idempotencia`? | Por contenido; no se escribe `guid` | `SELECT COUNT(*) FROM dbo.gra WHERE guid IS NOT NULL AND guid <> ''` |
| Q7 | ¿Escribe Sigrid en `dbo.log` al importar un gráfico? | No se escribe `log`; si da filas, se vuelve a proponer | `SELECT COUNT(*) FROM dbo.log WHERE tab = 'gra'` y `SELECT TOP (20) * FROM dbo.log WHERE tab = 'gra' ORDER BY ide DESC` |
| tipaso | ¿Qué código de `auxgra.tipaso` corresponde a `con.tip = 708`? | No se valida; la lista blanca de `gratipide` cubre | `SELECT ide, cod, tipaso FROM dbo.auxgra WHERE fecbaj = 0` |
| gra | Valores de las columnas no variables (y `res`/`gratipide` de la fila documental) | Se miden en T1 antes de escribir el constructor | Ver T1 en `tasks.md` |

## Fuera de alcance

Borrar o sustituir adjuntos; versionado (`graant`); `multipart` o ficheros de
decenas de MB; módulo `dog`/`condog`; abrir `sql/write` a la documental; portar
a `sigrid-api-v2`; escribir en `dbo.log` salvo que Q7 lo invierta.
